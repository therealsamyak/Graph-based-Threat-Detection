...
\begin{itemize}
\item \textit{Future work:} With 29 features across three levels, exhaustive ablation is computationally infeasible within our project timeframe. We plan to manually narrow the feature set to 3-5 features per flow type based on domain knowledge and preliminary signal analysis, then evaluate that reduced set across both graph scoring and baselines to ensure a direct comparison on identical features.
\end{itemize}
...
\begin{itemize}
\item \textit{Future work:} Weight optimization through grid search or Bayesian optimization once the feature set is finalized. The current weights are initial values set from domain knowledge and have not been tuned.
\end{itemize}
...
\begin{itemize}
\item \textit{Future work:} The methodology and results presented are a work in progress. Planned next steps include narrowing the feature set to 3-5 features per flow type and ensuring baselines operate on the same reduced feature set for direct comparison, weight optimization for scoring functions, hyperparameter tuning for baselines, statistical significance testing of results, and measuring computational cost (CPU usage, memory, wall-clock time) across all methods. The initial results should be interpreted as a proof of concept rather than an optimized detection system.
\end{itemize}
...
\hline
\end{tabular}
\caption{All eight methods on LANL-2015 (run \texttt{20260502\_165755}, 364{,}802-edge feature matrix, 305 red-team pairs). Graph methods evaluate threshold-dependent metrics in pair space (deduplicated source-destination pairs); tabular baselines evaluate in edge space. AUC is comparable across both spaces. \textit{(Figure by: Ibrahim Pehlivan)}}
