...
Edges are scored with a weighted sum whose terms depend on edge type. Each scoring function selects a small number of features from the edge feature vector described above and combines them with domain-knowledge-driven weights.
...
\hline
\end{tabular}
\caption{Five methods on LANL-2015 (run \texttt{20260503\_080850}, 512{,}529-edge combined graph, 305 red-team pairs). Graph scoring uses \texttt{is\_ntlm}, \texttt{is\_network\_logon}, \texttt{edge\_rarity} for auth edges; \texttt{edge\_rarity}, \texttt{is\_unusual\_dst\_port}, \texttt{protocol\_rarity} for flow edges. Baselines use the full edge feature vector with scikit-learn defaults. \textbf{Numbers not final.} \textit{(Table by: Ibrahim Pehlivan)}}
\label{tab:methods_lanl}
\end{table}

The combined graph method is strongest overall (AUC 0.954). Among baselines, Isolation Forest leads (AUC 0.940), within 0.015 of combined. The graph method's edge over the best baseline is small in AUC but larger in false-positive rate.
