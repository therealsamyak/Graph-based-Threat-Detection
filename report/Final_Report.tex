...
\caption{Modified Advanced-Persistent-Threat Model, showing Lateral Movement as the step after the attacker compromises one machine (Foothold). Adapted from Mamun et al.~\cite{APT-detection}. Placed by <someone>.}
\label{fig:lat_mvmt}
\end{figure}
...
Figure~\ref{fig:roc} presents the receiver operating characteristic (ROC) curves for the three detection methods evaluated on the LANL-Dataset-2015: flow-only, auth-only, and combined. The combined method achieves an AUC of 0.9456, substantially outperforming both the auth-only method (AUC = 0.9094) and the flow-only method (AUC = 0.0000). The combined method's curve lies closest to the top-left corner of the plot, indicating that it achieves higher true positive rates at lower false positive rates across the full range of decision thresholds.
...
This figure directly supports the paper's central hypothesis that no single log source provides sufficient signal to reliably identify lateral movement, but combining both log types into a unified graph enables detection that neither source can achieve alone. The auth-only method achieves moderate discriminative power (AUC = 0.9094) because authentication events capture credential-based lateral movement such as pass-the-hash and remote logon. However, the combined method's additional 0.036 AUC improvement demonstrates that network flow logs contribute complementary signal --- capturing connections that never produce authentication events, such as service exploitation and data exfiltration. The flow-only method's near-zero AUC confirms that network flow patterns alone, without authentication context, are insufficient to distinguish attack edges from the massive volume of benign enterprise traffic. Together, these results validate the design choice to construct a unified graph from both log sources rather than relying on a single data source.
