...
def plot_method_comparison(
    ...
    for col, title, y_label, suffix in zip(metrics_cols, metric_titles, y_labels, suffixes):
        ...
        ax.set_title(takeaway, fontsize=21, fontweight="bold", pad=15)
...
def plot_roc_curves(
    ...
    for variant in VARIANT_ORDER:
        ...
        ax.set_title(f"ROC analysis shows strong separability for {variant_label}", fontsize=21, fontweight="bold")
        ax.set_xlabel("False Positive Rate")
        ax.set_ylabel("True Positive Rate")
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.legend(fontsize=14, **_smart_legend_loc(ax, preferred="lower right"))
        ax.grid(alpha=0.3)
        fig.tight_layout()
...
def plot_radar_chart(
    ...
    for variant in VARIANT_ORDER:
        ...
        ax.set_ylim(0, 1)
        ax.set_title(f"Radar profile reveals method strengths across metrics for {variant_label}", fontsize=20, fontweight="bold", pad=20)
        ax.legend(fontsize=14, **_smart_legend_loc(ax, preferred="upper right"))
        ax.grid(alpha=0.3)
        fig.tight_layout()
