...
def plot_variant_heatmap(matrix: pd.DataFrame, output_dir: Path) -> None:
    ...
    titles = [
        "Detection accuracy varies across methods and attack variants",
        "F1 scores show method-dependent patterns across attack variants",
        "True positive rates show method-dependent patterns",
    ]
...
def plot_detection_counts(matrix: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set_ylabel("Count")
    ax.set_title("Predicted detections align closely with ground truth across methods", fontweight="bold", fontsize=13)
...
def plot_performance_tradeoff(matrix: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.legend(handles, labels, **_smart_legend_loc(ax), fontsize=8)
    ax.set_xlabel("Latency (ms)")
    ax.set_ylabel("AUC")
    ax.set_title("Higher accuracy methods require more processing time", fontweight="bold", fontsize=13)
...
def plot_metrics_summary(matrix: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set_title("All methods achieve strong detection performance with tradeoffs", fontweight="bold", fontsize=14, pad=20)
