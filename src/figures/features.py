...
def plot_feature_audit(audit_data: dict | None, output_dir: Path) -> None:
    ...
    for variant in ordered_variants:
        ...
        ax.set_title(f"Graph-based features dominate detection importance for {variant.replace('_', ' ').title()}", fontweight='bold', fontsize=12)
        ...
        ax.legend(handles=legend_handles, fontsize=7, **_smart_legend_loc(ax))
        fig.tight_layout()
...
def plot_ablation(analysis_data: dict | None, output_dir: Path) -> None:
    ...
    ax.set_ylim(0, 1.15)
    ax.set_title("Removing graph features causes the largest performance drop", fontweight='bold', fontsize=12)
...
def plot_feature_sweep(analysis_data: dict | None, output_dir: Path) -> None:
    ...
    for variant in ordered_variants:
        ...
        for bar, val in zip(bars, df["auc"].values):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.01, f"{val:.2f}", ha="center", va="bottom", fontsize=7)
        ax.set_title(f"Detection accuracy plateaus beyond 15 features for {variant.replace('_', ' ').title()}", fontweight='bold', fontsize=12)
