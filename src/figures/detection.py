...
def plot_score_distributions(
    ...
    for variant in variants:
        ...
        ax.set_yscale("log")
        ax.set_title(f"Anomaly scores cluster tightly for {variant}, separating benign from malicious traffic", fontweight="bold", fontsize=13)
        ax.set_xlabel("Anomaly Score")
        ax.set_ylabel("Count (log scale)")
        ax.legend(fontsize=7, **_smart_legend_loc(ax))
        fig.tight_layout()
...
def plot_detection_timeline(
    ...
    for variant, ts, scores, mask_rt in panel_data:
        ...
        ax.set_title(f"Detection events cluster around active attack phases in {variant.replace('_', ' ').title()}", fontweight="bold", fontsize=13)
        ax.set_xlabel("Time (seconds)")
        ax.set_ylabel("Anomaly Score")
        ax.legend(framealpha=0.9, fontsize=8, **_smart_legend_loc(ax))
        fig.tight_layout()
...
def plot_graph_statistics(results_dir: Path | None, output_dir: Path) -> None:
    ...
    ax.set_title("Graph topology differs significantly across attack variants", fontweight="bold", fontsize=13)
...
def plot_holdout_validation(analysis_data: dict | None, output_dir: Path) -> None:
    ...
    ax.set_title("Optimized model outperforms logistic regression on held-out data", fontweight="bold", fontsize=13)
    ax.legend(**_smart_legend_loc(ax))
    fig.tight_layout()
