...
def plot_method_comparison(
    ...
    if not results_list:
        ...
        ax.text(0.5, 0.5, "No results to display", ha="center", va="center",
                transform=ax.transAxes, fontsize=21, color="#95a5a6")
    ...
    for ax_idx, dataset in enumerate(datasets):
        ...
        if sub.empty:
            ax.text(0.5, 0.5, f"No data for {dataset}", ha="center", va="center",
                    transform=ax.transAxes, fontsize=18, color="#95a5a6")
            ...
            for bar, val in zip(bars, vals):
                if val > 0:
                    ax.text(
                        ...
                        f"{val:.3f}", ha="center", va="bottom", fontsize=14, color="#2c3e50",
                    )

        ax.set_xticks(x)
        ax.set_xticklabels(methods, rotation=25, ha="right", fontsize=14)
        ax.set_ylim(0, 1.15)
