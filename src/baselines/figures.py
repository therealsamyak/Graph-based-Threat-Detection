...
def _plot_f1(
    ...
    ax.set_xlabel("Data Variant", fontsize=20)
    ax.set_ylabel("F1 Score", fontsize=20)
    ax.set_title("Graph-based methods achieve highest F1 scores", fontsize=21, fontweight="bold")
    ax.set_xticks(x)
    ax.set_xticklabels([VARIANT_LABELS[v] for v in variants])
    ax.legend(fontsize=17, **_smart_legend_loc(ax))
    ax.set_yscale("log")
...
def _plot_auc(
    ...
    ax.set_title("AUC scores exceed 0.95 for all graph-based methods", fontsize=21, fontweight="bold")
    ax.set_xticks(x)
    ax.set_xticklabels([VARIANT_LABELS[v] for v in variants])
    ax.legend(fontsize=17, **_smart_legend_loc(ax))
...
def _plot_recall_fpr(
    ...
    ax.set_title("High recall achieved with minimal false positive cost", fontsize=21, fontweight="bold")
    ax.legend(fontsize=17, **_smart_legend_loc(ax))
    ax.grid(alpha=0.3)
...
def _plot_radar(
    ...
    ax.set_ylim(0, 1)
    ax.set_title("Radar profiles reveal complementary method strengths", fontsize=21, fontweight="bold", pad=20)
    ax.legend(fontsize=17, **_smart_legend_loc(ax, preferred="upper right"))
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
def _plot_detected_pairs(
    ...
    for i, variant in enumerate(variants):
        ...
        ax.set_title(variant.replace("_", " ").title(), fontsize=20)
        ax.set_xlabel("Detection Method", fontsize=18)
        ax.set_ylabel("Detected Pairs", fontsize=18)
        ax.tick_params(axis="x", rotation=45)
        if i == 0:
            ax.legend(fontsize=15, **_smart_legend_loc(ax))

    plt.suptitle("Detection coverage varies across methods and variants", fontsize=22, fontweight="bold")
    plt.tight_layout()
