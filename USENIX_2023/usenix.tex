...
\begin{itemize}
\item Narrow feature set to 3--5 per flow type based on domain knowledge and signal analysis
\item Run ablation on reduced feature set across graph scoring and baselines for direct comparison
\end{itemize}
\end{itemize}
...
\item \textit{Future work:}
\begin{itemize}
\item Optimize scoring weights via grid search or Bayesian optimization once feature set is finalized
\item Current weights are domain-knowledge defaults and have not been tuned
\end{itemize}
\end{itemize}
...
\item \textit{Future work:}
\begin{itemize}
\item Tune hyperparameters for Isolation Forest and One-Class SVM (currently scikit-learn defaults)
\end{itemize}
\end{itemize}
...
\item \textit{Future work:}
\begin{itemize}
\item Narrow feature set to 3--5 per flow type; ensure baselines use same reduced set for direct comparison
\item Optimize scoring weights via grid search or Bayesian optimization
\item Tune baseline hyperparameters (currently defaults)
...
\item Current results are a proof of concept, not an optimized system
\end{itemize}
\end{itemize}
...
Figure~\ref{fig:scores} shows the anomaly score distribution for all 512,529 edges in the combined method on a log-scale y-axis, with KDE curves overlaid. Scores range from 0.0 to 1.48, with a clear bimodal distribution: a large cluster at 0.0--1.0 (baseline edges) and a smaller cluster at 1.0--1.48 (path-boosted edges). Red-team edges (orange, n=305) concentrate in the high-score cluster, while baseline edges (green, n=512,224) span both clusters. The dashed orange line marks the detection threshold at 1.41.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{score_distribution.png}
\caption{Anomaly score distribution for the combined method (log scale, with KDE curves). Score range [0.00, 1.48]. Red-team edges (orange, n=305) concentrate at high scores. Baseline edges (green, n=512,224) span the full range. The dashed line marks the detection threshold (1.41). \textit{(Figure by: Wesley Gunawan)}}
\label{fig:scores}
\end{figure}

This figure shows the scoring function separates attack edges from baseline: red-team edges concentrate in the high-score cluster (1.0--1.48) created by path boosting. The bimodal distribution reflects the path boost mechanism (Section~4.4), which adds $0.1 \times \text{path\_score}$ to edges in top-50 suspicious paths, pushing them above 1.0. The threshold at 1.41 separates the most suspicious edges from the rest. This highlights a key trade-off: path boosting creates a distinct high-score cluster that captures attack edges, but also pulls some baseline edges into that cluster, inflating false positives.
...
\hline
\end{tabular}
\caption{Five methods evaluated on LANL-2015. The combined graph method achieves the highest AUC (0.954), while Isolation Forest is the strongest baseline (AUC 0.940). The gap is small in AUC but more pronounced in FPR (0.020 pair-space vs. 0.050 edge-space). \textbf{Numbers not final.} \textit{(Table by: Ibrahim Pehlivan)}}
...
\subsection{Earlier Results Draft:}
\begin{itemize}
