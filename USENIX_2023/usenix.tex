...
\caption{Anomaly score distribution for the combined method (log scale). Score range [0.00, 1.48]. Red-team edges (orange, n=305) concentrate at high scores. Baseline edges (green, n=512,224) span the full range. The dashed line marks the detection threshold (1.41). \textit{(Figure by: Wesley Gunawan)}}
\label{fig:scores}
\end{figure}

This figure shows the scoring function separates attack edges from baseline: red-team edges concentrate at higher scores. The path boost mechanism (Section~4.4) creates a high-score cluster above 1.0, where most red-team edges appear. The threshold at 1.41 separates the most suspicious edges from the rest.
