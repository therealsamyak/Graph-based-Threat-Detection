...
\caption{Anomaly score distribution for the combined method (log scale). Red-team edges (orange, n=305) have mean score 0.98. Baseline edges (green, n=512,224) have mean score 0.45, with a spike at 1.0 from path boosting. \textit{(Figure by: Wesley Gunawan)}}
\label{fig:scores}
\end{figure}

This figure shows the scoring function separates attack edges from baseline: red-team edges score 2.2$\times$ higher on average (0.98 vs. 0.45). The spike at 1.0 in the baseline distribution is caused by path boosting (Section~4.4), which adds $0.1 \times \text{path\_score}$ to edges in top-50 suspicious paths. This highlights a key trade-off: path boosting improves recall but inflates false positives.
