...
Figure~\ref{fig:roc} presents the receiver operating characteristic (ROC) curves for the detection methods evaluated on the LANL-Dataset-2015. The plot shows five curves: three graph-based methods (flow-only, auth-only, combined), two unsupervised baselines (OneClassSVM, Isolation Forest), and a random baseline diagonal. The OneClassSVM curve (orange) bows above the diagonal, indicating modest discriminative power, while all three graph-based methods and Isolation Forest hug the diagonal, indicating near-random performance at the chosen threshold.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{roc_curves.png}
\caption{ROC curves for all detection methods on LANL-2015. OneClassSVM (orange) shows modest discriminative power above the random baseline, while graph-based methods (flow-only, auth-only, combined) and Isolation Forest hug the diagonal. \textit{(Figure by: Wesley Gunawan)}}
\label{fig:roc}
\end{figure}

This figure reveals a key challenge in lateral movement detection: the graph-based scoring function, while theoretically sound, produces edge scores that do not cleanly separate attack edges from benign edges at a fixed percentile threshold. All three graph methods (flow-only, auth-only, combined) produce nearly identical ROC curves hugging the diagonal, suggesting that the hand-crafted scoring weights (Equations~1--2) require tuning to achieve meaningful separation. The OneClassSVM baseline's curve above the diagonal indicates that learned anomaly detection on the edge feature matrix can achieve better ranking than the rule-based scoring. This result motivates future work on optimizing scoring weights via supervised learning or adaptive thresholding, and suggests that the combined graph structure provides a richer feature space for downstream classifiers even when the hand-crafted scoring alone is insufficient.
